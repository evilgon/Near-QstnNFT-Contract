...
impl Contract {
    ...
    pub(crate) fn assert_owner(&self) {
        assert_eq!(&env::predecessor_account_id(), &self.owner_id, "{}", "ERR_NOT_OWNER_ACCOUNT");
    }
...
impl FungibleTokenReceiver for Contract {
    fn ft_on_transfer(
        ...
        log!("Sending {} ... ", ft_token_id);
        if msg == "Deposit" {
            log!("--> Deposit amount is {} by {}", amount.0, sender_id);
            let event_data = format!("{} deposited : {}", sender_id, amount.0);
            env::log_str(&event_data);
        }
